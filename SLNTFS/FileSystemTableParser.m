...
-(id)init
{
	@throw([NSException exceptionWithName:EX_WRONG_INIT reason:@"Use -(id)initWithCopyPath:(NSString*)path instead" userInfo:nil]);
}
...
-(void)fstabRead
{
	...
	NSArray* lines = [[NSString stringWithContentsOfFile:_copyPath encoding:NSASCIIStringEncoding error:NULL] componentsSeparatedByString:@"\n"];
...
-(BOOL)saveWithAuthorization:(SFAuthorization*)auth
{
	...
	if (![lines writeToFile:_copyPath atomically:YES encoding:NSASCIIStringEncoding error:NULL])
	{
		NSLog(@"Failed to write to %@\n", _copyPath);
		return NO;
	}
	char* args[3] = {(char*)[_copyPath cStringUsingEncoding:NSASCIIStringEncoding], (char*)[FSTAB_PATH cStringUsingEncoding:NSASCIIStringEncoding], NULL};
	if (AuthorizationExecuteWithPrivileges([auth authorizationRef], [BIN_CP_PATH cStringUsingEncoding:NSASCIIStringEncoding], kAuthorizationFlagDefaults, args, NULL) != errAuthorizationSuccess)
	{
...
{
	char* args[5] = {"-f", (char*)[FSTAB_PATH cStringUsingEncoding:NSASCIIStringEncoding], (char*)[_copyPath cStringUsingEncoding:NSASCIIStringEncoding], (char*)[_backupPath cStringUsingEncoding:NSASCIIStringEncoding], NULL};
	AuthorizationExecuteWithPrivileges([auth authorizationRef], [BIN_RM_PATH cStringUsingEncoding:NSASCIIStringEncoding], kAuthorizationFlagDefaults, args, NULL);
	[self copyForReading];
}
