...
{
	char* cpArgs[4] = {"-f", "/Library/PreferencePanes/SLNTFS.prefPane/Contents/Resources/com.juicybinary.slntfsDaemon.plist", "/Library/LaunchDaemons/com.juicybinary.slntfsDaemon.plist", NULL};
	OSStatus ret = AuthorizationExecuteWithPrivileges([auth authorizationRef], [BIN_CP_PATH cStringUsingEncoding:NSASCIIStringEncoding], kAuthorizationFlagDefaults, cpArgs, NULL);
	ALog(@"%d", ret);
}
...
+(void)launchDaemonsAtLogin:(BOOL)state withAuthorization:(SFAuthorization*)auth
{
	if (state)
		...
		[DaemonHandler disableDaemonAtLoginWithAuthorization:auth];
}
